...
penCoin <- penguins %>%
  drop_na() %>%
  mutate(coins = round(exp(bill_length_mm*0.05 + bill_depth_mm * 0.02)) + rpois(n(), 1),
         coins2 = round(exp(bill_length_mm*0.05 + bill_depth_mm * 0.02) + rpois(n(), 1)),
...
summary(coinMod2)
testDF <- data.frame(bill_length_mm = c(35, 40, 45, 50),
                     bill_depth_mm = c(15, 17, 19, 21))
predict(coinMod1, testDF)
predict(coinMod2, testDF, type = "response")
exp(testDF$bill_length_mm * 0.05 + testDF$bill_depth_mm * 0.02) + 1
```
