...
Let $X_1, X_2, \ldots, X_n$ be a random sample from a uniform distribution on $[0,1]$ (i.e., iid with $\mu = 1/2$, $\sigma = 1/12$).
...
$$
>- If we want to get within $\epsilon$ with probability $p$, we should choose $n$ so that $n \geq \frac{1}{\epsilon(1-p)}$.
...
```{r, eval=FALSE}
n <- 1/(0.01*0.05)
try2000 <- replicate(10000, mean(g(runif(n))))
hist(try2000)
abline(v = c(0.7468241-0.01, 0.7468241+0.01), col="red")
```
