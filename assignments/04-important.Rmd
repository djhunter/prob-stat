...
4. Do Exercise 27 on page 22 of [GS]. If $X$ represents the test scores, then $Z = (X - \mu)/\sigma$ is standard normal. Therefore you can compute the required probabilities using `pnorm`. (For example, $\mu + \sigma < X$ implies $[(\mu + \sigma) - \mu]/\sigma < (X - \mu)/\sigma$, so you can just compute $P(1 < Z)$ using `pnorm`.)
